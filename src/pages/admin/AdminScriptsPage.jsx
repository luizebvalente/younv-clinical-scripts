...
const AdminScriptsPage = () => {
  ...
  const loadScripts = async () => {
    try {
      ...
      if (hasPermission('super_admin')) {
        // Super admin can see all scripts from all clinics
        // For now, we'll load scripts from the current user's clinic
        // In a real implementation, you'd need to modify scriptService to get all scripts
        const clinicId = userData?.clinicId || 'clinica-sao-lucas';
        const result = await scriptService.getScriptsByClinic(clinicId, { limitCount: 1000 });
        allScripts = result.scripts;
      } else {
        // Regular users see only their clinic's scripts
      ...
      setError('Erro ao carregar scripts: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    loadScripts();
  }, [searchTerm, selectedCategory, sortBy, userData]);
  ...
  const handleBulkAction = async (action) => {
    try {
      ...
      if (action === 'delete') {
        await Promise.all(selectedScripts.map(scriptId => scriptService.deleteScript(scriptId)));
      } else if (action === 'activate') {
        await Promise.all(selectedScripts.map(scriptId => 
          scriptService.updateScript(scriptId, { isActive: true })
        ));
      } else if (action === 'deactivate') {
        await Promise.all(selectedScripts.map(scriptId => 
          scriptService.updateScript(scriptId, { isActive: false })
        ));
      }
      ...
      setError('Erro ao executar ação em lote: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCloneScript = async (script) => {
    try {
      setIsLoading(true);
      await scriptService.duplicateScript(script.id);
      await loadScripts(); // Reload scripts
    } catch (error) {
      console.error('Erro ao clonar script:', error);
      setError('Erro ao clonar script: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
