...
const AdminDashboardPage = () => {
  ...
  // Load admin dashboard data from Firebase
  const loadAdminDashboardData = async () => {
    try {
      setIsLoading(true);
      setError(null);
      
      // For super admin, load global stats
      if (hasPermission('super_admin')) {
        // For now, we'll load data from the current clinic
        // In a real implementation, you'd need to aggregate data from all clinics
        const clinicId = userData?.clinicId || 'clinica-sao-lucas';
        
        // Get script statistics
        const scriptStats = await scriptService.getScriptStats(clinicId);
        
        // Get recent scripts for activity
        ...
        const topCategories = Object.entries(scriptStats.byCategory).map(([categoryId, count]) => {
          const categoryNames = {
          ...
          return {
            ...
            count,
            percentage: scriptStats.total > 0 ? (count / scriptStats.total * 100).toFixed(1) : 0
          };
        }).sort((a, b) => b.count - a.count);

        // Mock recent activity from scripts
        const mockRecentActivity = scriptsResult.scripts.slice(0, 4).map((script, index) => ({
          id: script.id,
          type: 'script_created',
          title: 'Script atualizado',
          description: `Script "${script.title}" foi modificado`,
          clinic: 'Clínica São Lucas',
          timestamp: script.updatedAt?.toDate?.() || script.updatedAt || new Date(),
          icon: FileText,
          color: 'text-green-600 bg-green-100'
        }));

        setStats({
          totalClinics: 1, // For now, just one clinic
          totalUsers: 3, // Demo users
          totalScripts: scriptStats.total,
          activeScripts: scriptStats.total, // Assuming all are active
          totalUsage: scriptStats.total * 10, // Mock usage count
          monthlyGrowth: 15.3, // Mock growth
          topCategories
        });

        setRecentActivity(mockRecentActivity);
      } else {
        // Regular admin sees only their clinic data
        ...
        if (clinicId) {
          ...
          setStats({
            ...
            totalUsers: 1,
            totalScripts: scriptStats.total,
            activeScripts: scriptStats.total,
            totalUsage: scriptStats.total * 5,
            monthlyGrowth: 10.0,
            topCategories: []
          });
        }
