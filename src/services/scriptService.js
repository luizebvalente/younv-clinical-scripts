...
class ScriptService {
  ...
  async getScriptsByCategory(categoryId, clinicId) {
    try {
      console.log('ScriptService - getScriptsByCategory iniciado', { categoryId, clinicId });
      
      if (!authService.canAccessClinic(clinicId)) {
        console.log('ScriptService - Acesso negado à clínica:', clinicId);
        throw new Error('Acesso negado a esta clínica.');
      }

      console.log('ScriptService - Criando query...');
      const q = query(
        collection(db, this.collectionName),
      ...
      console.log('ScriptService - Executando query...');
      const querySnapshot = await getDocs(q);
      const scripts = [];
      ...
      console.log('ScriptService - Scripts encontrados:', scripts.length, scripts);
      return scripts;
    } catch (error) {
